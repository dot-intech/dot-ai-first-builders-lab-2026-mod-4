...
FR-017/FR-023/FR-024 (amend de spec, ítem de `BACKLOG.md` "subir el límite
...
- [x] T077 [US1] Actualizar `tests/unit/session.test.ts` para que los tests
...
- [x] T078 [US1] Actualizar `tests/integration/session.test.ts` (test "la
...
- [x] T079 [US1] Cambiar la constante `INACTIVIDAD_MAX_MS` de
      `8 * 60 * 60 * 1000` a `24 * 60 * 60 * 1000` en
      `lib/auth/session.ts:4`, para pasar FR-006/RNF-06 (partial).
- [x] T080 [US1] Correr `npm test` (unit+integration+contract) y confirmar
      que la suite completa está en verde tras T077-T079, para verificar
      FR-006/RNF-06 (missing).
- [x] T081 [US1] Gap no detectado por `/speckit-analyze` ni `/speckit-converge`:
      `tests/integration/auth-dashboard.test.ts` (línea ~117) también tenía
      "8h" hardcodeado (nombre del test y `interval '8 hours 1 minute'`),
      encontrado recién al correr `npm test` tras T079 y ver un tercer
      archivo fallar. Actualizado a 24h, mismo patrón que T078.
